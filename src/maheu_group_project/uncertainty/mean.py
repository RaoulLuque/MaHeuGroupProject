...
def mean_capacity(truck_history: dict[tuple[Weekday, Location, Location, int], list[Truck]]) -> dict[tuple[Weekday, Location, Location, int], float]:
    """
    ...
    for dict_key, trucks in truck_history.items():
        ...
        mean_capacity_res[dict_key] = mean
    return mean_capacity_res
