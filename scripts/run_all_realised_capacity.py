...
SOLVERS = [ SolverType.LOWER_BOUND_UNCAPACITATED_FLOW, SolverType.FLOW, SolverType.OLD_FLOW]
...
def run_on_all_data_from_first_dataset():
    for solver in SOLVERS:
        ...
        with open(os.path.join(results_dir, f"{current_time}_{solver}_result.txt"), 'w') as result_file:
            with open(os.path.join(results_dir, f"{current_time}_{solver}_result_pretty.txt"), 'w') as pretty_result_file:
